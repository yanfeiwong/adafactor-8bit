...
- **量化精度与稳定性**：Adafactor 的二阶矩（方差）始终非负，所以我们使用 `UINT8 (0~255)` 进行映射。相比传统的 8bit 优化器映射 `INT8 (-127~127)`，有效精度提升了一倍，在训练中有更优的数值稳定性。

## 安装

这个项目采用 JIT (Just-In-Time) 编译，无预编译二进制文件。如果 CUDA 编译失败，优化器会自动回退到纯 PyTorch 实现。请确保你的环境中已安装 `torch` 和 `ninja`，并配置好了 CUDA 编译器（如 MSVC 或 GCC）。
