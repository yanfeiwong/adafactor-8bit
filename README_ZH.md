...
- **APOLLO 低秩投影与 Fira 减震器**：内置了可选的随机子空间投影路径，在低秩子空间内估计梯度缩放因子以加速收敛，同时搭配了 Fira 范数增长限制器（Norm-Growth Limiter）。
...
## APOLLO 低秩子空间投影
启用 APOLLO 路径，在极低显存占用的低秩子空间内计算梯度缩放因子。与 Adafactor 标准的行列分解（假设空间独立）相比，APOLLO 利用随机子空间投影捕获更丰富的协方差信息，在保持极低显存开销的同时，往往能带来更快的收敛速度。
